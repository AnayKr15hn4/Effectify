...
const authorStatusMap: Record<string, string> = {
  "Anay Krishna": "big-boss",
  "John Doe": "platinum",
  "Jane Smith": "gold",
  "Alex Green": "emerald",
  "Eli White": "silver",
...
const getStatusClass = (author: string): string => {
  const status = authorStatusMap[author]; // read from effect.author
...
export const EffectCard: React.FC<EffectCardProps> = ({
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <span
            className={`px-3 py-1 text-xs font-bold rounded-full border capitalize font-secondary ${
              categoryBadgeColors[effect.category]
            }`}
          >
            {effect.category}
          </span>
  ...
  Created by{" "}
  <span
    className={`font-semibold status-shine ${getStatusClass(effect.author)}`}
  >
    {effect.author}
  </span>
</p>
